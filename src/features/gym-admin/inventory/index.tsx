...
const ItemFormModal: React.FC<{
  ...
  const handleSave = async () => {
    const nextErrors = {
      ...
      quantity: form.quantity.trim() ? '' : 'Quantity is required.',
    };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={item ? 'Edit Item' : 'Add Inventory Item'} size="lg"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="md:col-span-2"><Input label="Item Name *" value={form.name} onChange={s('name')} error={errors.name} placeholder="e.g. Rubber Dumbbell Set" /></div>
        <Input label="SKU *" value={form.sku} onChange={s('sku')} error={errors.sku} placeholder="EQ-DUMB-001" />
        <Select label="Category" value={form.category} onChange={sv('category')}
          options={CATEGORIES.map(c => ({ value: c, label: c }))} />
        <Input label="Quantity *" type="number" value={form.quantity} onChange={s('quantity')} error={errors.quantity} placeholder="0" />
        ...
        <div className="md:col-span-2"><Input label="Selling Price (₹) — leave 0 if not for sale" type="number" value={form.sellingPrice} onChange={s('sellingPrice')} placeholder="0" /></div>
        ...
        <div className="md:col-span-2">
          <Textarea label="Description" rows={2} value={form.description} onChange={s('description')} placeholder="Brief description..." />
        </div>
      </div>
    </Modal>
  );
