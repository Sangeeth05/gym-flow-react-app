...
const SettingsPage: React.FC = () => {
  ...
  const sg = (k: string) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
    setGymForm(f => ({ ...f, [k]: e.target.value }));
  const sgv = (k: string) => (value: string) => setGymForm(f => ({ ...f, [k]: value }));
            ...
            <>
              <Card className="p-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2"><Input label="Address" value={gymForm.address} onChange={sg('address')} /></div>
                  <Input label="City" value={gymForm.city} onChange={sg('city')} />
                  <Input label="State" value={gymForm.state} onChange={sg('state')} />
                  <Select label="Country" value={gymForm.country} onChange={sgv('country')}
                    showSearch
                    optionFilterProp="label"
                    options={['India','UAE','USA','UK','Singapore','Malaysia','Australia'].map(c => ({ value: c, label: c }))} />
                  <Select label="Currency" value={gymForm.currency} onChange={sgv('currency')}
                    options={[
                  ...
                  <div className="md:col-span-2">
                    <Textarea label="Description" rows={3} value={gymForm.description} onChange={sg('description')} />
                  </div>
                </div>
              </Card>
            ...
            <>
              <Card className="p-5">
                ...
                <div className="mt-4 pt-4 border-t border-[var(--color-bg-600)]">
                  ...
                  <Input type="number" className="w-28" value={notif.expiryDaysBefore}
