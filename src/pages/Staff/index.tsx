    ...
    <Modal isOpen={isOpen} onClose={onClose} title={staff ? 'Edit Staff Member' : 'Add Staff Member'} size="lg"
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select className="input-field" value={form.role} onChange={s('role')}>
            {ROLES.map(r => <option key={r} value={r}>{r}</option>)}
          </select>
        </div>
        <Input label="Monthly Salary (₹)" type="number" value={form.salary} onChange={s('salary')} placeholder="30000" />
        <Input label="Join Date" type="date" value={form.joinDate} onChange={s('joinDate')} />
        <div>
          <label className="label">Schedule / Shift</label>
          <input className="input-field" value={form.schedule} onChange={s('schedule')} placeholder="e.g. Mon–Sat, 6AM–2PM" />
        </div>
