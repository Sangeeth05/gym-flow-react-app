...
import { Badge, SearchInput, StatCard, Spinner, EmptyState, Modal, Button, Input, Select } from '../../components/ui';
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Record Payment" size="md"
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2">
          ...
          <select className="input-field" value={form.memberId} onChange={set('memberId')}>
            <option value="">— Walk-in / No member —</option>
            {members.map(m => <option key={m.id} value={m.id}>{m.firstName} {m.lastName} ({m.memberId})</option>)}
          </select>
        </div>
        <div>
        ...
        <div className="col-span-2">
          <Input label="Description *" value={form.description} onChange={set('description')} placeholder="e.g. Premium Plan - May 2024" />
        </div>
        <div>
