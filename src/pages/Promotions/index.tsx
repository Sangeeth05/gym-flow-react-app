...
import { Tag, Plus, Copy, CheckCircle, Edit, Trash2, Eye } from 'lucide-react';
import { Badge, StatCard, Spinner, EmptyState, Modal, Button, Input, ConfirmDialog } from '../../components/ui';
import { promoApi } from '../../api/services';
import { PromoCode, PromoStatus, DiscountType } from '../../types';
...
const PromoFormModal: React.FC<{ isOpen: boolean; onClose: () => void; promo?: PromoCode | null; onSave: () => void }> = ({ isOpen, onClose, promo, onSave }) => {
  ...
  const handleSave = async () => {
    if (!form.code || !form.discountValue || !form.validFrom || !form.validTo) { toast.error('Fill required fields'); return; }
    setLoading(true);
    try {
      ...
      <div className="space-y-4">
        <div>
          ...
          <div className="flex gap-2">
            <input className="input-field font-mono uppercase tracking-widest" value={form.code}
              onChange={s('code')} placeholder="WELCOME50" maxLength={20} />
            <button type="button" onClick={genCode} className="btn-secondary whitespace-nowrap text-xs px-3">Auto Generate</button>
          </div>
        </div>
        <div>
          <label className="label">Description</label>
          <textarea className="input-field" rows={2} value={form.description} onChange={s('description')} placeholder="e.g. Welcome offer for new members" />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="label">Discount Type *</label>
            <select className="input-field" value={form.discountType} onChange={s('discountType')}>
          ...
          <div>
            ...
            <input className="input-field" type="number" value={form.discountValue} onChange={s('discountValue')}
          ...
          <div>
            ...
            <input className="input-field" type="number" value={form.minPurchase} onChange={s('minPurchase')} placeholder="0" />
          </div>
          <div>
            <label className="label">Max Uses *</label>
            <input className="input-field" type="number" value={form.maxUses} onChange={s('maxUses')} placeholder="100" />
          </div>
          <div>
            <label className="label">Valid From *</label>
            <input className="input-field" type="date" value={form.validFrom} onChange={s('validFrom')} />
          </div>
          <div>
            <label className="label">Valid To *</label>
            <input className="input-field" type="date" value={form.validTo} onChange={s('validTo')} />
          </div>
        </div>
        <div>
          <label className="label">Applicable For</label>
          <select className="input-field" value={form.applicableFor} onChange={s('applicableFor')}>
            <option value="All">All Members</option>
            <option value="NewMembers">New Members Only</option>
            <option value="RenewalOnly">Renewal Only</option>
