...
import { Badge, SearchInput, StatCard, Spinner, EmptyState, Modal, Button, Input, ConfirmDialog } from '../../components/ui';
...
const ProductFormModal: React.FC<{ isOpen: boolean; onClose: () => void; product?: Product | null; onSave: () => void }> = ({ isOpen, onClose, product, onSave }) => {
  ...
  const handleSave = async () => {
    if (!form.name || !form.price || !form.sku) { toast.error('Fill required fields'); return; }
    setLoading(true);
    try {
      const data = {
        ...form,
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={product ? 'Edit Product' : 'Add Product'} size="lg"
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2"><Input label="Product Name *" value={form.name} onChange={s('name')} placeholder="e.g. Whey Protein Isolate 2kg" /></div>
        ...
        <Input label="SKU *" value={form.sku} onChange={s('sku')} placeholder="MB-WPI-2KG" />
        <Input label="Stock Quantity" type="number" value={form.stock} onChange={s('stock')} placeholder="0" />
        <Input label="Price (₹) *" type="number" value={form.price} onChange={s('price')} placeholder="0.00" />
        ...
        <div className="col-span-2">
          ...
          <textarea className="input-field" rows={2} value={form.description} onChange={s('description')} placeholder="Product description..." />
        </div>
        <div className="col-span-2">
