...
import {
  Users, CreditCard, Package, TrendingUp, AlertTriangle,
...
import { dashboardApi, dashboardSummaryApi } from '../../api/services';
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      <div>
        ...
        <p className="page-subtitle">{user?.gymName} — Here's what's happening today</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Active Members" value={stats?.activeMembers.toLocaleString() ?? 0} change={8.2}
          icon={<Users className="w-5 h-5 text-brand-400" />} iconBg="bg-brand-500/10"
          subtext={`+${stats?.newMembersThisMonth} this month`} />
        <StatCard title="Monthly Revenue" value={formatCurrency(stats?.monthlyRevenue ?? 0)} change={stats?.revenueGrowth}
          icon={<TrendingUp className="w-5 h-5 text-emerald-400" />} iconBg="bg-emerald-500/10" />
        <StatCard title="Today's Check-ins" value={stats?.todayCheckIns ?? 0}
          icon={<CheckCircle2 className="w-5 h-5 text-blue-400" />} iconBg="bg-blue-500/10"
          subtext={`${stats?.occupancyRate}% occupancy`} />
        <StatCard title="Expiring Soon" value={stats?.expiringSoon ?? 0}
          icon={<AlertTriangle className="w-5 h-5 text-yellow-400" />} iconBg="bg-yellow-500/10"
          subtext="Members in 7 days" />
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Members" value={stats?.totalMembers.toLocaleString() ?? 0}
          icon={<Users className="w-5 h-5 text-purple-400" />} iconBg="bg-purple-500/10" />
        <StatCard title="Pending Payments" value={formatCurrency(stats?.pendingPayments ?? 0)}
          icon={<CreditCard className="w-5 h-5 text-red-400" />} iconBg="bg-red-500/10" />
        <StatCard title="Low Stock Items" value={stats?.lowStockItems ?? 0}
          icon={<Package className="w-5 h-5 text-yellow-400" />} iconBg="bg-yellow-500/10" />
        <StatCard title="Total Staff" value={stats?.totalStaff ?? 0}
          icon={<UserCheck className="w-5 h-5 text-teal-400" />} iconBg="bg-teal-500/10" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <Card className="lg:col-span-2 p-5">
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-4 text-xs">
              ...
              <span className="flex items-center gap-1.5 text-slate-400">
                <span className="w-2 h-2 rounded-full bg-blue-500" />Expenses
              </span>
            </div>
          </div>
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={revenue} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
              <defs>
              ...
              <Area type="monotone" dataKey="revenue" name="Revenue" stroke="#f97316" strokeWidth={2}
                fill="url(#revGrad)" dot={false} activeDot={{ r: 4, fill: '#f97316' }} />
              <Area type="monotone" dataKey="expenses" name="Expenses" stroke="#3b82f6" strokeWidth={2}
                fill="url(#expGrad)" dot={false} activeDot={{ r: 4, fill: '#3b82f6' }} />
            </AreaChart>
          </ResponsiveContainer>
        </Card>
        ...
        <Card className="p-5">
          ...
          <ResponsiveContainer width="100%" height={160}>
            <PieChart>
              <Pie data={revenueByType.length ? revenueByType : [
                { type: 'Membership', amount: 198000 },
                { type: 'Products', amount: 54000 },
                { type: 'PT Sessions', amount: 24000 },
                { type: 'Other', amount: 8500 },
              ]}
                dataKey="amount" nameKey="type" cx="50%" cy="50%" innerRadius={50} outerRadius={75}
                paddingAngle={3}
              >
                {PIE_COLORS.map((color, i) => <Cell key={i} fill={color} />)}
              </Pie>
              <Tooltip formatter={(v: any) => formatCurrency(v)} contentStyle={{
                backgroundColor: '#1a1a24', border: '1px solid #2d2d3d', borderRadius: 8, fontSize: 12
              }} />
            </PieChart>
          </ResponsiveContainer>
          <div className="space-y-1.5 mt-2">
            {['Membership', 'Products', 'PT Sessions', 'Other'].map((label, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card className="p-5">
          ...
          <div className="space-y-3">
            {[
              { label: 'Active Members', value: stats?.activeMembers ?? 0, total: stats?.totalMembers ?? 1, color: 'bg-brand-500' },
              { label: 'Paid This Month', value: 89, total: 100, color: 'bg-emerald-500' },
              { label: 'Occupancy Today', value: stats?.occupancyRate ?? 0, total: 100, color: 'bg-blue-500' },
              { label: 'Products In Stock', value: 148, total: stats?.totalProducts ?? 156, color: 'bg-purple-500' },
            ].map(item => (
              <div key={item.label}>
                ...
                <div className="h-1.5 bg-dark-600 rounded-full overflow-hidden">
                  <div className={`h-full ${item.color} rounded-full transition-all duration-500`}
                    style={{ width: `${Math.min(100, Math.round((item.value / (item.total || 1)) * 100))}%` }} />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              { label: 'Check-ins', value: stats?.todayCheckIns?.toString() ?? '0', icon: '✅' },
